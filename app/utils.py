...
LGD = 0.45  # loss given default — industry standard assumption for unsecured consumer credit


# Must be defined here (not just in the training notebook) so joblib can
# unpickle xgboost_best.pkl, which was saved as an instance of this class.
class CalibratedXGB:
    def __init__(self, base_model, scaler):
        self.base_model = base_model
        self.scaler = scaler
        self.estimator = base_model  # for SHAP compatibility
...
def load_artifacts():
    """Load model, preprocessor, metadata, and SHAP explainer once. Cached by caller."""
    ...
    base_xgb = model.estimator  # raw XGBoost inside the calibrated wrapper
...
def build_raw_input_df(inputs: dict) -> pd.DataFrame:
    """
    Convert the 14 form inputs into a single-row DataFrame matching
    the schema the preprocessor pipeline was fit on.
    """
    row = {
    ...
    # Engineer credit_age_months exactly as in the training pipeline
...
def score_applicant(raw_df: pd.DataFrame, model, preprocessor, threshold: float):
    """Run the full pipeline: preprocess -> predict -> decision."""
    ...
    risk_score = int(round((1 - prob) * 100))  # 100 = lowest risk, 0 = highest risk

    return prob, decision, risk_score, X_df


def get_shap_explanation(X_df, explainer, feature_names_clean):
    """Compute SHAP values for a single processed row and return a shap.Explanation."""
